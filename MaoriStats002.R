# start  R script #
# Maori Language stats from NZ Censuses, Peter J Keegan updated 12 Jan 2015
#
#
#
library(ggplot2)
...
Speakers <- c(NA, 129048, 130482, 131613,125352,116000) # no language qu pre 1996
#
...
#
rm(plot04h, plot04i, plot04j, plot04k)
#
# Section 5 #
#
...
#
Iwi <-c("Ngapuhi","Ngati Porou","Ngai Tahu / Kai Tahu","Waikato","Ngati Tuwharetoa","Ngati Maniapoto","Tuhoe","Ngati Kahungunu ki Te Wairoa",
        "Te Arawa","Ngati Kahungunu, region unspecified","Te Rarawa","Ngati Awa","Te Atiawa (Taranaki)","Ngati Raukawa (ki te Tonga)",
        "Ngati Whatua","Te Whanau-a-Apanui","Ngaiterangi","Whakatohea","Tainui","Te Ati Hau Nui-a-Paparangi",
        "Ngati Kahungunu ki Heretaunga","Ngati Raukawa (Waikato)")
M2001 <-c(26646,19431,5148,12636,8958,8031,12363,4332,5241,7278,4107,4746,2070,3084,3447,4353,3609,3474,3786,3342,2460,1497)
E2001 <-c(76005,41907,33639,23118,20184,19041,16923,10236,11325,17481,7347,8247,7953,7860,8580,5562,5853,6438,8940,5433,4428,3609)
T2001 <-c(102651,61338,38787,35754,29142,27072,29286,14568,16566,24759,11454,12993,10023,10944,12027,9915,9462,9912,12726,8775,6888,5106)
Per2001 <-c(26.0,31.7,13.3,35.3,30.7,29.7,42.2,29.7,31.6,29.4,35.9,36.5,20.7,28.2,28.7,43.9,38.1,35.0,29.8,38.1,35.7,29.3)
M2006 <-c(29733,21153,5973,11412,10428,9375,13344,5535,7458,4977,4959,5133,2391,3444,4194,4821,4209,4014,4128,3684,3084,2244)
E2006 <-c(92805,51084,43110,22203,24300,24291,19500,15432,15978,13920,9960,10164,10383,9786,10581,6993,7986,8106,10221,6807,6447,5862)
T2006 <-c(122538,72237,49083,33615,34728,33666,32844,20967,23436,18897,14919,15297,12774,13230,14775,11814,12195,12120,14349,10491,9531,8106)
Per2006 <-c(24.3,29.3,12.2,33.9,30.0,27.8,40.6,26.4,31.8,26.3,33.2,33.6,18.7,26.0,28.4,40.8,34.5,33.1,28.8,35.1,32.4,27.7)
M2013 <-c(28053,19425,6270,12879,10011,9429,13413,5460,5670,4422,5064,5262,2679,3678,3732,5058,4347,3819,2544,3738,3300,2763)
E2013 <-c(97302,51447,48132,27252,25776,25902,21522,15492,14046,14049,11349,10884,12477,11343,10995,7797,8496,8334,9264,7947,7656,7254)
T2013 <-c(125355,70872,54402,40131,35787,35331,34935,20952,19716,18471,16413,16146,15156,15021,14727,12855,12843,12153,11808,11685,10956,10017)
Per2013 <-c(22.4,27.4,11.5,32.1,28.0,26.7,38.4,26.1,28.8,23.9,30.9,32.6,17.7,24.5,25.3,39.3,33.8,31.4,21.5,32.0,30.1,27.6)
#
d008 <- data.frame(Iwi,M2001,E2001,T2001,Per2001,M2006,E2006,T2006,Per2006,M2013,E2013,T2013,Per2013)
#
rm(Iwi,M2001,E2001,T2001,Per2001,M2006,E2006,T2006,Per2006,M2013,E2013,T2013,Per2013)
#
d008a <- d008[c(1,13,9,5)]
#
d008b <- melt(d008a, id.vars="Iwi")
names(d008b) <- c("Iwi", "Year", "Percent")
#
d008c <- d008[c(1,10,11)]
d008d <- melt(d008c, id.vars="Iwi")
names(d008d) <- c("Iwi", "Language", "Number")
#
# plot08a <- ggplot(d008d, aes(x=Iwi, y=Number, fill=Language)) + geom_bar(stat="identity")
# plot08a+ theme_bw(base_size = 18) + ggtitle("Maori/English Speakers by Major Iwi") + coord_flip()
#
#
...
plot08f + scale_x_discrete(limits=c("Ngati Raukawa (Waikato)","Ngati Kahungunu ki Heretaunga",
                                    ...
                                    "Ngati Raukawa (ki te Tonga)", "Te Atiawa (Taranaki)",
                                    "Ngati Awa", "Te Rarawa",
                                    "Ngati Kahungunu, region unspecified",
                                    "Te Arawa", "Ngati Kahungunu ki Te Wairoa",
                                    "Tuhoe", "Ngati Maniapoto",
                                    "Ngati Tuwharetoa", "Waikato",
                                    "Ngai Tahu / Kai Tahu",
                                    "Ngati Porou",
