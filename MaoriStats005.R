# start R script
# Maori Language stats from NZ Censuses, Peter J Keegan last updated 3 Jan 2023
# all data created in the script, self contained but not best way to do things.
# Done using R via RStudio (now Posit) in OSX, if using Windows etc. check Unicode OK.
# object names mostly lower case, or sometimes lowerCamelCase
# d.. usually indicates dataframe, plot.. indicates plot object
#
# note I am adding 2018 & 2023 data where possible
#
...
#
# Section 1 Overview
# Create vectors of NZ Census (Maori) Data
#
year <- c(1991, 1996, 2001, 2006, 2013, 2018, 2023)
ethnicity <- c(434847, 523371, 526281, 565329, 598605, 775836, 887493)
descent <- c(511278, 579714, 604110, 643977, 668724, 869850, 978246)
speakers <- c(NA, 129048, 130482, 131613, 125352, 159645, 181953) # no lang. qu pre 1996
#
...
plot01b <- plot01a + ggtitle(paste0("Figure 1: New Zealand Census 1991-2023",
...
plot01f <- plot01e + scale_x_continuous(name = "Census Year",
  breaks = c(1991, 1996, 2001, 2006, 2013, 2018, 2023), expand = c(.02, 0))
plot01g <- plot01f + theme (plot.title = element_text(size = 20))
plot01g + theme(legend.justification=c(1,.1), legend.position.inside=c(1,.4))
#
#
...
plot02f + theme(legend.justification=c(1,0), legend.position.inside =c(.8,.7))
#
